resource "kubernetes_namespace" "auth-adminer" {
  metadata {
...
resource "kubernetes_deployment" "auth-adminer" {
  metadata {
    name      = "${var.database_name}-adminer"
    namespace = kubernetes_namespace.auth-adminer.metadata[0].name
  }
...
resource "kubernetes_service" "auth_adminer_service" {
  metadata {
    name      = "${var.database_name}-adminer"
    namespace = kubernetes_namespace.auth-adminer.metadata[0].name
  }
